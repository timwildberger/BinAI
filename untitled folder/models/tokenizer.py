...
class AsmTokenizer:
    ...
    def tokenize(self, text):
        # Basic regex patterns for assembly tokens:
        #  - Hex numbers: 0x followed by hex digits
        #  - Decimal numbers: digits
        #  - Identifiers (labels, instructions): word characters and underscores
        #  - Special characters: comma, brackets, etc.
        pattern = r"""(
            0x[0-9a-fA-F]+        |  # hex numbers
            \d+                   |  # decimal numbers
            [a-zA-Z_][a-zA-Z0-9_]*|  # identifiers (mnemonics, labels)
            [,()\[\]]                # special punctuation chars
        )"""

        # Find all matching tokens, ignoring whitespace
